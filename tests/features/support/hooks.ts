...
Before({ tags: 'not @api' }, async function (this: World, scenario: ITestCaseHookParameter) {
  ...
  // Add a small stagger to reduce grid load spikes
  const baseDelay = Math.max(2000, 15000 / totalWorkers);
  const jitter = Math.floor(Math.random() * 2000) - 1000;
  const delayMs = workerIndex * baseDelay + jitter;
  ...
  // Pick a browser type (Chromium, Firefox, WebKit rotation)
  const browsers = [chromium, firefox, webkit];
  const browserType = browsers[workerIndex % browsers.length];
  console.log(`🌐 Worker ${workerIndex} launching ${browserType.name()} browser`);

  // Launch browser + context + page via World (handles login state)
  await this.openBrowser(browserType);

  await this.attach(
      `🌐 ${browserType.name()} launched with state by worker ${workerIndex} (PID ${this.pid})`,
      'text/plain'
  );
});
// Before({ tags: 'not @api' }, async function (this: World, scenario: ITestCaseHookParameter) {
...
//   const totalWorkers = parseInt(process.env.CUCUMBER_PARALLEL_TOTAL || process.env.THREADS || '4', 10);
//
//   // Dynamic stagger timing to reduce load spikes
//   const baseDelay = Math.max(2000, 15000 / totalWorkers);
//   const jitter = Math.floor(Math.random() * 2000) - 1000;
//   const delayMs = workerIndex * baseDelay + jitter;
//
...
//   }
//
//   // 🧩 Choose browser dynamically (rotates between Chromium, Firefox, WebKit)
//   const browsers = [chromium, firefox, webkit];
//   const browserType = browsers[workerIndex % browsers.length];
//   console.log(`🌐 Worker ${workerIndex} launching ${browserType.name()} browser`);
//
//   // Launch or connect via World
//   await this.openBrowser(browserType);
//   await this.attach(`🌐 ${browserType.name()} launched for scenario with PID ${this.pid}`, 'text/plain');
//
//   // 🗂️ Pick random pre-generated auth state file (if exists)
//   const statesDir = path.resolve('tests/auth-states');
//   let selectedState: string | undefined;
//
//   if (fs.existsSync(statesDir)) {
...
//   }
//
//   // 🚀 If selected state exists, inject it manually into context
//   if (selectedState && this.context) {
//     try {
//       const stateData = JSON.parse(fs.readFileSync(selectedState, 'utf8'));
...
//           }
//         } catch (err) {
//           console.warn('⚠️ Failed to apply storageState manually:', err);
//         }
//       }, { state: stateData });
//       console.log(`💾 Applied storage state from: ${path.basename(selectedState)}`);
//     } catch (err) {
//       console.warn(`⚠️ Could not apply storage state ${selectedState}:`, err);
//     }
//   }
//
//   await this.attach(`🧭 Using browser + context created by World for: ${scenario.pickle.name}`, 'text/plain');
// });

// ---------- Per-scenario cleanup ----------
After({ tags: 'not @api' }, async function (this: World) {
  try {
    // 🧹 Clear in-page storage
    if (this.page) {
      try {
        await this.page.evaluate(async () => {
          localStorage.clear();
