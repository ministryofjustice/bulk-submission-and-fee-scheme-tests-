...
const sanitizeForCode = (str: string) =>
    str.replace(/[^\p{L}0-9 \-’'&]/gu, '').toUpperCase();
...
const fetchProviderSchedules = async (office: string, caseStartDate: Date) => {
  ...
  try {
    const response = await axios.get(
        `${PROVIDER_API}/${office}/schedules?effectiveDate=${formattedDate}&areaOfLaw=${AREA_OF_LAW}`,
        {
          headers: {
...
const generateFile = async (
  ...
  content += `SCHEDULE,submissionPeriod=${submissionPeriod},areaOfLaw=${AREA_OF_LAW},scheduleNum=${scheduleNum}\n`;

  for (let i = 0; i < outcomesCount; i++) {
    const o = await generateOutcome(office, i);
    const claimOverride = options.claims?.[i];
    const feeCode = claimOverride?.feeCode ?? o.fee_code;
