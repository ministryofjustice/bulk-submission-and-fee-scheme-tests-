...
const generateOutcome = async (office: string, caseNum: number) => {
    ...
    const maxCaseStart = new Date('2020-12-31');
    ...
    if (!caseStartDate) throw new Error(`Unable to generate valid case start date within provider schedules for office ${office}`);
    ...
    return {
        ...
        crime_matter_type: pad(Number(randomFrom(['1','2','3'])), 2), // '01', '02', '03'
        disability: randomFrom(['NCD','MOB','DEA','HEA','VIS','BLI','MHC','LDD','COG','ILL','OTH','UKN','PHY','SEN']),
...
const generateFile = async (fileName: string, outcomesCount: number, fileType: 'txt' | 'csv') => {
    ...
    for (let i = 1; i <= outcomesCount; i++) {
        ...
        content += `OUTCOME,FEE_CODE=${feeCode},matterType=${feeCode},UFN=${o.ufn},STAGE_REACHED_CODE=${o.stage_reached_code},CLIENT_FORENAME=${o.client_forename},CLIENT_SURNAME=${o.client_surname},GENDER=${o.gender},ETHNICITY=${o.ethnicity},DISABILITY=${o.disability},CASE_START_DATE=${o.case_start_date},PROFIT_COST=${o.profit_cost},DISBURSEMENTS_AMOUNT=${o.disbursements_amount},DISBURSEMENTS_VAT=${o.disbursements_vat},VAT_INDICATOR=${o.vat_indicator},TRAVEL_COSTS=${o.travel_costs},OUTCOME_CODE=${o.outcome_code},CRIME_MATTER_TYPE=${o.crime_matter_type},TRAVEL_WAITING_COSTS=${o.travel_waiting_costs},WORK_CONCLUDED_DATE=${o.work_concluded_date},NO_OF_SUSPECTS=${o.no_of_suspects},NO_OF_POLICE_STATION=${o.no_of_police_station},POLICE_STATION=${o.police_station},DUTY_SOLICITOR=${o.duty_solicitor},YOUTH_COURT=${o.youth_court},SCHEME_ID=${o.scheme_id},DSCC_NUMBER=${o.dscc_number}\n`;
    }
