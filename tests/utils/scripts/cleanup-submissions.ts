...
// ---------- 4️⃣ Exported Runner (to be called from hooks.ts) ----------
export async function runSubmissionCleanup() {
    const providerUser = 'Test.User-submit-a-bulk-claim-auto-test@devl.justice.gov.uk';
    ...
    try {
        ...
        if (submissionIdsToDelete.length === 0) {
            ...
            console.log(
                `🧾 Found ${submissionIdsToDelete.length} submission(s) for ${providerUser} in the past 4 days.`
            );
            await cleanSubmissionData(AppDataSource, submissionIdsToDelete);
        }
    } catch (error) {
