...
Given('I start from a clean logged-in state', async function (this: World) {
    ...
    console.log('ℹ️ Skipping Sign-out check — Azure AD session not present after cookie rewrite.');
    console.log('ℹ️ Not calling logoutAndWipe — storageState is source of truth.');
    ...
    // Recreate logged-in context from rewritten storage
    const { context, page } = await recreateLoggedInContext({
        browser: this.browser!,
        baseURL: process.env.UI_BASE_URL!,
        storageStatePath: this.workerStoragePath,
