...
// ✅ NEW STEP: boolean assertion for steps like
// And the JSON path "escapeCaseFlag" should be boolean TRUE
Then(
    ...
    async function (this: World, jsonPath: string, expectedWord: string) {
        ...
        if (normalized === undefined) {
            throw new Error(
                `Value at path "${jsonPath}" is not coercible to boolean: ${JSON.stringify(actual)}`
            );
        }

        const expectedBool = expected === 'true';
        expect(normalized).toBe(expectedBool);
    }
);
